...
const Dashboard: React.FC<{ onLogout: () => void }> = ({ onLogout }) => {
  ...
  const [filters, setFilters] = useState<FilterCriteria>({
    ...
    education: 'all', hasWife: 'all', hasChildren: 'all',
    foreignElement: 'all', familyStatus: 'all', marital: 'all'
  });
  ...
  // Logic sắp xếp chuẩn Tiếng Việt (Tách tên)
  const sortedPersonnel = useMemo(() => {
    ...
    if (sortBy === 'name') {
      return list.sort((a, b) => {
        ...
        const lastNameComparison = nameA.localeCompare(nameB, 'vi', { sensitivity: 'base' });
        
        // Nếu tên trùng nhau thì xét đến cả họ tên
    ...
    if (sortBy === 'age') {
      return list.sort((a, b) => new Date(a.ngay_sinh).getTime() - new Date(b.ngay_sinh).getTime());
    }
  ...
  const exportCSV = () => {
    const headers = ['Họ Tên', 'CCCD', 'Đơn Vị', 'Cấp Bậc', 'Ngày Sinh', 'Nhập ngũ', 'Trình độ'];
    const rows = personnel.map(p => [p.ho_ten, p.cccd, p.don_vi, p.cap_bac, p.ngay_sinh, p.nhap_ngu_ngay, p.trinh_do_van_hoa]);
    const csvContent = "\uFEFF" + [headers, ...rows].map(e => e.join(",")).join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const link = document.createElement("a");
    const url = URL.createObjectURL(blob);
    link.setAttribute("href", url);
    link.setAttribute("download", `danh_sach_quan_nhan_${new Date().toLocaleDateString('vi-VN')}.csv`);
  ...
  return (
    <div className="h-screen bg-[#F8FAFC] flex font-sans text-slate-800 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-h-0 overflow-hidden">
        ...
        {/* Content Area */}
        <main className="flex-1 overflow-y-auto p-8 min-h-0 relative">
          
          {activeTab === 'overview' && dashboardStats && (
            <div className="space-y-10 animate-fade-in">
              <div className="grid grid-cols-4 gap-6">
                <StatCard title="Tổng Quân Số" value={dashboardStats.total} icon={Users} color="bg-blue-50 text-blue-600" subValue={dashboardStats.newRecruits > 0 ? `+${dashboardStats.newRecruits} mới` : null} />
          ...
          {activeTab === 'personnel' && (
            <div className="space-y-4 animate-fade-in pb-20">
              <div className="flex justify-between items-center px-2">
                <h2 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Danh sách tìm thấy: <span className="text-[#14452F] font-black">{sortedPersonnel.length}</span></h2>
                <div className="flex gap-2">
                   <div className="flex bg-white border rounded-xl overflow-hidden p-1">
                      <button onClick={() => setSortBy('name')} className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${sortBy === 'name' ? 'bg-[#14452F] text-white' : 'text-slate-400'}`}>Tên (A-Z)</button>
                      <button onClick={() => setSortBy('age')} className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${sortBy === 'age' ? 'bg-[#14452F] text-white' : 'text-slate-400'}`}>Tuổi</button>
                      <button onClick={() => setSortBy('enlistment')} className={`px-4 py-1.5 rounded-lg text-[9px] font-black uppercase transition-all ${sortBy === 'enlistment' ? 'bg-[#14452F] text-white' : 'text-slate-400'}`}>Nhập ngũ</button>
                   </div>
                   <button 
                      onClick={() => setShowAdvancedFilter(!showAdvancedFilter)} 
                      className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase flex items-center gap-2 transition-all border ${showAdvancedFilter ? 'bg-[#14452F] text-white border-[#14452F]' : 'bg-white text-slate-400 border-slate-200'}`}
                   >
                     <Filter size={14}/> {showAdvancedFilter ? 'Đóng bộ lọc' : 'Lọc nâng cao'}
                   </button>
                </div>
              </div>
              ...
              {showAdvancedFilter && (
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-xl animate-slide-down grid grid-cols-4 gap-6 mb-8">
                  <div>
                    <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase mb-3"><ShieldCheck size={14}/> Chính trị</label>
                    <select className="w-full p-3 bg-slate-50 rounded-xl outline-none font-bold text-xs" value={filters.political} onChange={e => setFilters({...filters, political: e.target.value})}>
                  ...
                  <div>
                    <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase mb-3"><GraduationCap size={14}/> Trình độ</label>
                    <select className="w-full p-3 bg-slate-50 rounded-xl outline-none font-bold text-xs" value={filters.education} onChange={e => setFilters({...filters, education: e.target.value})}>
                      <option value="all">Tất cả</option>
                      <option value="dai_hoc_cao_dang">Đại học / Cao đẳng</option>
                    </select>
                  </div>
                  <div>
                    <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase mb-3"><Heart size={14}/> Hôn nhân</label>
                    <select className="w-full p-3 bg-slate-50 rounded-xl outline-none font-bold text-xs" value={filters.hasWife} onChange={e => setFilters({...filters, hasWife: e.target.value})}>
                      <option value="all">Tất cả</option>
                      <option value="yes">Đã có vợ</option>
                      <option value="no">Chưa có vợ</option>
                    </select>
                  </div>
                  <div>
                    <label className="flex items-center gap-2 text-[10px] font-black text-slate-400 uppercase mb-3"><AlertTriangle size={14}/> Cảnh báo</label>
                    <select className="w-full p-3 bg-slate-50 rounded-xl outline-none font-bold text-xs" value={filters.security} onChange={e => setFilters({...filters, security: e.target.value})}>
                      <option value="all">Tất cả</option>
                      <option value="vay_no">Có vay nợ</option>
                      <option value="vi_pham">Có vi phạm</option>
                    </select>
                  </div>
                  <div className="col-span-4 flex justify-end gap-3 pt-2">
                    <button onClick={resetFilters} className="px-6 py-2.5 bg-slate-100 text-slate-400 rounded-xl font-black text-[10px] uppercase hover:bg-slate-200">Xóa bộ lọc</button>
                    <button onClick={() => setShowAdvancedFilter(false)} className="px-6 py-2.5 bg-[#14452F] text-white rounded-xl font-black text-[10px] uppercase shadow-lg">Áp dụng lọc</button>
                  </div>
                </div>
              )}
              ...
              <div className="grid gap-3">
                {sortedPersonnel.map((p, i) => (
                    <div key={p.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex items-center gap-4 group hover:border-[#14452F]/20 transition-all hover:shadow-md animate-fade-in" style={{ animationDelay: `${i * 30}ms` }}>
                    <div className="w-14 h-14 bg-slate-50 rounded-2xl flex items-center justify-center font-black text-[#14452F] text-xs border border-slate-100 overflow-hidden shrink-0 shadow-inner group-hover:scale-105 transition-transform">
                        {p.anh_dai_dien ? <img src={p.anh_dai_dien} className="w-full h-full object-cover" /> : p.ho_ten.charAt(0)}
                    </div>
                    <div className="flex-1 grid grid-cols-12 gap-4">
                        <div className="col-span-4">
                        <p className="text-xs font-black text-slate-800 uppercase leading-tight group-hover:text-[#14452F] transition-colors">{p.ho_ten}</p>
                        <p className="text-[9px] text-slate-400 font-bold mt-1 tracking-widest">{p.cccd}</p>
                        </div>
                        <div className="col-span-3">
                        <p className="text-[10px] font-black text-slate-700 uppercase">{p.cap_bac}</p>
                        <p className="text-[9px] font-black text-slate-400 uppercase truncate mt-0.5">{p.chuc_vu || 'N/A'}</p>
                        </div>
                        <div className="col-span-5 flex flex-wrap items-center gap-2">
                        <span className="px-2 py-0.5 bg-green-50 text-green-700 rounded-lg text-[8px] font-black uppercase border border-green-100">{p.don_vi}</span>
                        {p.vao_dang_ngay && <span className="px-2 py-0.5 bg-red-50 text-red-600 rounded-lg text-[8px] font-black uppercase border border-red-100 flex items-center gap-1"><ShieldCheck size={10}/> Đảng Viên</span>}
                        {p.trinh_do_van_hoa?.toLowerCase().includes('đại học') && <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-lg text-[8px] font-black uppercase border border-blue-100 flex items-center gap-1"><GraduationCap size={10}/> Đại học</span>}
                        {p.quan_he_gia_dinh?.vo && <span className="px-2 py-0.5 bg-pink-50 text-pink-700 rounded-lg text-[8px] font-black uppercase border border-pink-100 flex items-center gap-1"><Heart size={10}/> Có vợ</span>}
                        {p.tai_chinh_suc_khoe?.vay_no?.co_khong && <span className="px-2 py-0.5 bg-amber-50 text-amber-700 rounded-lg text-[8px] font-black uppercase border border-amber-100 flex items-center gap-1"><AlertTriangle size={10}/> Nợ</span>}
                        </div>
                    </div>
                    <div className="flex gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => setViewingPerson(p)} className="p-2.5 text-blue-600 hover:bg-blue-50 rounded-xl transition-all"><Eye size={18} /></button>
                        <button onClick={() => { setEditingPerson(p); setIsFormOpen(true); }} className="p-2.5 text-[#14452F] hover:bg-green-50 rounded-xl transition-all"><Edit3 size={18} /></button>
                        <button onClick={async () => { if(confirm('Xóa hồ sơ này?')) { await db.deletePersonnel(p.id); loadData(); } }} className="p-2.5 text-red-400 hover:bg-red-50 rounded-xl transition-all"><Trash2 size={18} /></button>
                    </div>
                    </div>
                ))}
              </div>
              ...
              {sortedPersonnel.length === 0 && (
                <div className="py-20 text-center space-y-4 bg-white rounded-[3rem] border border-dashed border-slate-200">
                   <FilterX size={48} className="mx-auto text-slate-200" />
                   <p className="text-xs font-black text-slate-300 uppercase tracking-widest">Không tìm thấy quân nhân nào phù hợp</p>
                   <button onClick={resetFilters} className="px-6 py-2 bg-[#14452F] text-white rounded-xl text-[10px] font-black uppercase shadow-lg">Xóa bộ lọc</button>
                </div>
              )}
            </div>
          )}
